...
| Calls | Step 92 background notification complete | Firestore session/state API, atomic offer/accept, role-specific ICE paths, strict rules, WebRTC engine, STUN, and optional local TURN config exist; Step 90-91 calling flows implemented; Step 92 background notification via Cloudflare Worker + FCM implemented; Step 93 foreground service polish remains. |
